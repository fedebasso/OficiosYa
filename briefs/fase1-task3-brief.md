# Fase1-IA Task 3: ticketService llama Edge Function

**Working directory:** `C:\Users\fede8\Documents\OficiosYa\oficioYa`

## Contexto

Reemplazar `src/services/ai/ticketService.ts` por una implementación que llama a la Edge Function de Supabase. En modo demo (Supabase placeholder), usar el mock. Si la Edge Function devuelve 422 (no_match), lanzar `Error('NO_MATCH')` — NO usar el mock como fallback en ese caso. Para otros errores de red/servidor, fallback silencioso al mock.

## Reemplazar `src/services/ai/ticketService.ts` completo
...
1. Reemplazar `src/services/ai/ticketService.ts` con el contenido exacto de arriba
2. Verificar en dev server `http://localhost:5177/ticket` — modo demo funciona (mock, ~2.5s)
3. Commit:
...
- Solo modificar `src/services/ai/ticketService.ts`
- La firma pública `analyzeTicket(input: TicketInput): Promise<GeneratedTicket>` no cambia
- En `IS_DEMO_MODE`: usar mock, no llamar red
- En `res.status === 422`: lanzar `new Error('NO_MATCH')` — NO fallback
- Cualquier otro error: fallback silencioso al mock

## Reporte

Escribir en `C:\Users\fede8\Documents\OficiosYa\briefs\fase1-task3-report.md`.
